...
class DSTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ✅ Use first real token, not [CLS] (which is token 101)
        input_ids = tokens["input_ids"].squeeze()
        label = input_ids[1] if len(input_ids) > 1 else input_ids[0]  # Fallback if too short

        return frames, input_ids, tokens["attention_mask"].squeeze(), label
...
for epoch in range(5):
    ...
    for img_feat, input_ids, att_mask, labels in train_loader:
        optimizer.zero_grad()
        output = model(img_feat, input_ids, att_mask)  # [B, vocab_size]
        loss = criterion(output, labels)  # labels: [B]
        loss.backward()
        optimizer.step()
